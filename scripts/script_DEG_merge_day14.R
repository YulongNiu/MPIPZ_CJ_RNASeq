...
design(degres) <- ~sv1 + sv2 + sv3 + sv4 + sv5 + sv6 + Conditions
...
res <- cbind.data.frame(as.matrix(mcols(degres)[, 1:10]), assay(rld), stringsAsFactors = FALSE) %>%
  ...
  select(ID, Gene : Description, Col0_FeCl3_HK_Day15_Rep1 : f6h1_FeCl3_Live_vs_f6h1_FeCl3_HK_log2FoldChange) %>%
...
ggsave('PCA_mergeDay15_sva.pdf', width = 12)
ggsave('PCA_mergeDay15_sva.jpg', width = 12)

write_csv(res, 'eachGroup_mergeDay15.csv')
save(degres, rldData, file = 'eachGroup_mergeDay15.RData')
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##################################################################
